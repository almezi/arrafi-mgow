...
function tryMe(source){
    ...
    $.post("<?= base_url('index.php/caraka/simpan_kegiatan') ?>", {nama_kegiatan: radio.val()}, function(response){});
}

function saveDetail(){
	var data = $("#form-cek").serialize();
	$.ajax({
	url: "http://localhost/arrafi/index.php/caraka/simpan_kegiatan",
	type:"POST",
	data:data,
  ...
  var cboxes = document.getElementsByName('idk[]');
    ...
    for (var i=0; i<len; i++) {
		if (cboxes[i].checked){
			...
			$.ajax({
				url: 'http://localhost/arrafi/index.php/caraka/hapus_kegiatan',
				type: 'post',
				data: 'value='+cboxes[i].value,
				success: function(output){
					//alert('success, server says '+output);
