...
class M_guru extends CI_Model {
    ...
    {
		$query = $this->db->query("select thnajar from ajar ");
		$thnajar ='';
	...
	//$thnajar=$this->thnajar();
		$query=$this->db->query
		("select distinct * from kelas k
		join bab b on(k.idkelas=b.idkelas)
		where b.idmapel='".$_GET['idmapel']."'");
		return $query;
	}
	...
	//$thnajar=$this->thnajar();
		$query=$this->db->query
		("select * from bab where idbab='".$_GET['idbab']."'");
		return $query;
	}
	...
	public function guru_info_pilihsiswa()
	{
		...
		$query=$this->db->query
		("select distinct s.*,k.nama kelas from siswa s
	...
	public function guru_siswaeval($idbab)
	{
		...
		AND b.idbab='".$idbab."' and e.thnajar='".$thnajar."' and e.semester='".$semester."'
		");
		}
		return $query;
	}

	public function guru_siswapmp($idkelas, $semester, $idmapel)
	{
	//$thnajar=$this->thnajar();
		/*$query=$this->db->query
		("select s.nama,s.nis,if((select ROUND(nilai,2) from pmp where nis=s.nis and idmapel='".$_GET['idmapel']."' and semester=".$_GET['semester']." and thnajar=".$_GET['thnajar'].") is null,'',(select ROUND(nilai,2) from pmp where ks.nis=s.nis and idmapel='".$_GET['idmapel']."' and semester=".$_GET['semester']." and thnajar=".$_GET['thnajar'].")) as nilai, if((select round(skor,2) from pmp where ks.nis=s.nis and idmapel=".$_GET['idmapel']." and semester=".$_GET['semester']." and thnajar=".$_GET['thnajar'].") is null,'',(select round(skor,2) from pmp where ks.nis=s.nis and idmapel=".$_GET['idmapel']." and semester=".$_GET['semester']." and thnajar=".$_GET['thnajar'].")) as skor from siswa s
		join kelas_siswa ks on(ks.nis=s.nis)
		join kelas k on(ks.idkelas=k.idkelas)
		...
		join kelas_siswa ks on(ks.nis=s.nis)
		join kelas k on(ks.idkelas=k.idkelas)
		where p.idkelas='".$idkelas."' and p.semester='".$semester."' and p.idmapel='".$idmapel."'");
		return $query;
	}
	...
	public function guru_remed_siswaeval()
	{
		...
		if ($idbab != null or $_GET['idbab'] == null){
		$query=$this->db->query
		("select DISTINCT s.nis, s.nama, b.nama bab, b.idbab idbab, if(round(r.remidi,2)=0,'',round(r.remidi,2)) as nilai from siswa s
		join kelas_siswa ks on(ks.nis=s.nis)
		join kelas k on(ks.idkelas=k.idkelas)
	...
	public function guru_remed_siswapmp($thnajar,$semester,$idmapel,$idkelas)
	{
	$thnajar=$this->thnajar();
		$query=$this->db->query
		...
		where r.nilai < m.kkm and k.idkelas='".$idkelas."'
		and m.idmapel='".$idmapel."' and ks.thnajar='".$thnajar."' and r.thnajar='".$thnajar."' and r.semester='".$semester."'
		");
		return $query;
	}
...
//		$thnajar=$_GET['thnjar'];
		...
		join kelas_siswa ks on(k.idkelas=ks.idkelas)
		where b.idmapel='".$idmapel."' and b.idkelas='".$idkelas."'	and b.semester='".$semester."' and b.thnajar='".$thnajar."'
		");
		return $query;
	}
	...
	public function wali_akhirdkn($nis, $thnajar, $semester,$idmapel)
	{
		//$thnajar=$this->thnajar();
		$query=$this->db->query
		("select DISTINCT round(if(p.remidi=0,p.nilai,
		if(((p.remidi+p.nilai)/2)<m.kkm and p.remidi >= m.kkm,m.kkm,((p.remidi+p.nilai)/2))),2) as pmp,
		round(avg(if(e.remidi=0,e.nilai,
		if(((e.remidi+e.nilai)/2)<m.kkm and e.remidi >= m.kkm,m.kkm,((e.remidi+e.nilai)/2)))),2) as rata,
		round(((0.8*avg(if(e.remidi=0,e.nilai,
		...
		from evaluasi e join bab b on(e.idbab=b.idbab)
		join mapel m on(b.idmapel=m.idmapel)
		join pmp p on(m.idmapel=p.idmapel)
		where e.nis='".$nis."'
		and p.nis='".$nis."'
		and m.idmapel='".$idmapel."'
	...
	public function ortu_pilih_anak()
	{
		$id=$this->session->userdata('username');
		$thnajar=$this->thnajar();
		$query=$this->db->query
	...
	public function ortu_pilih_mapel()
	{
		...
		join kelas_siswa ks on(ks.idkelas=k.idkelas)
		join siswa s on(s.nis=ks.nis)
		where s.nis='".$_GET['nis']."' and b.semester='".$_GET['semester']."' and b.thnajar='".$_GET['thnajar']."'");
		}
		return $query;
	}

	public function ortu_jadwaleval()
	{
	$thnajar=$this->thnajar();
		$query=$this->db->query
	...
	public function ortu_jadwalpmp()
	{
	$thnajar=$this->thnajar();
		$query=$this->db->query
	...
	$query=$this->db->query
		("select distinct b.semester from bab b
			...
			join kelas_siswa ks on(ks.idkelas=k.idkelas)
			join siswa s on(s.nis=ks.nis)
		where s.nis='".$_GET['nis']."' and b.thnajar='".$_GET['thnajar']."' order by b.semester asc");
	}
	return $query;
	}
	...
	function tambah_nilai_siswa($object){
		return $this->db->insert('evaluasi',$object);
	}
}
