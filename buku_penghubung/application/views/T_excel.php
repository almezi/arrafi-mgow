...
<?php if($Tawal == $Takhir) { ?>
	...
	header("Content-Disposition: attachment; filename= Laporan Excel Wali Kelas $kelas dari tanggal $Tawal.xls");
...
<?php } else { ?>
	<?php
	header("Content-type: application/octet-stream");
	header("Content-Disposition: attachment; filename= Laporan Excel Wali Kelas $kelas dari tanggal $Tawal - $Takhir.xls");
	header("Pragma: no-cache");
	header("Expires: 0");
	?>
<?php }  ?>
...
<tr>
	<?php foreach($listSaranWali as $row) { ?>
		...
		Kelas : <?php echo $row->kelas ?> <?php $nip=$row->nip ?> </th>
	<?php } ?>
</tr>
<tr>
...
?>                                          
	<tr align=center>
		...
		<td><?php $listSiswa=$this->db->query("
		select s.nama from guru g join wali w on(g.nip=w.nip) join kelas k on(w.idkelas=k.idkelas) 
		join kelas_siswa ks on(ks.idkelas=k.idkelas) join siswa s on(ks.nis=s.nis) join ortu o on(s.idortu=o.idortu) 
		where w.nip=$nip and o.nama like '%$ortu[$i]%' ");?>
